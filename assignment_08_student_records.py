...
def add_student(students):
    ...
    try:
        ...
        print("Invalid number input. Student creation cancelled.")
...
def display_all_students(students):
    ...
    print("-" * 55)
    print(f"{'Name':<18}{'ID':<14}{'Scores':<15}{'Average':<8}")
    print("-" * 55)

    for student in students:
        scores = student["scores"]
        scores_str = ", ".join(str(int(s) if s.is_integer() else s) for s in scores)
        avg = sum(scores) / len(scores) if scores else 0.0
        print(f"{student['name']:<18}{str(student['id']):<14}{scores_str:<15}{avg:.2f}")
